...
if __name__ == "__main__":
    ...
    my_args = parser.add_argument_group()
    ...
    training_args = pl.Trainer.add_argparse_args(training_args)
    ...
    config.network_architecture.latent_dim_s = ((d.parameters.l_max + 1) ** 2) * (2 * d.parameters.freq_max - 2)
    ...
    # print((d.parameters.l_max + 1) ** 2)
    # print(((d.parameters.l_max + 1) ** 2) * d.parameters.freq_max)
    enc_config.latent_dim = ((d.parameters.l_max + 1) ** 2) * (2 * d.parameters.freq_max - 1)

    h = encoder.forward_conv_stack(x.s_t, preserve_graph_structure=False)
    
    NT = config.dataset.parameters.T
    ...
    model = t_encoder_ptl # get_lightning_module(config, dm.dataset)            
    ...
    main(model, datamodule, trainer, config.mlflow)
